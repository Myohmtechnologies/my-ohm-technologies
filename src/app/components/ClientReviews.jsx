    ...
    <section className="avis-du-client">
      <div className="btn-nos-instal">
        ...
        <button>
          ...
          <img src="/images/svg/icons8-right-arrow-32.png" alt="Right arrow icon" />
        </button>
      </div>
        ...
        {displayedRealizations.length > 0 ? (
          displayedRealizations.map((realisation) => (
            <div key={realisation._id} className="card-project">
              <img
                src={realisation.mainImage ? realisation.mainImage : '/images/placeholder-image.png'}
                alt={`Installation panneaux solaires - ${realisation.title}`}
              />
              <div className="card-content">
                <div className="badge">{realisation.puissanceMax} kWc</div>
                <span className="date">{new Date(realisation.date).toLocaleDateString('fr-FR')}</span>
                <h3>{realisation.title}</h3>
                <a href={`/installations-details/${realisation.slug}`} className="details-link">Afficher les détails</a>
